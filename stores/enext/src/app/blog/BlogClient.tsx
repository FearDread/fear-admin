...
export default function BlogClient({ initialPosts, initialCategories }: BlogClientProps) {
    ...
    const [sortBy] = useState<'asc' | 'desc'>('desc');
    ...
    const categories = (categoriesData ?? initialCategories).filter((c) => c.visible !== false);

    const [subscribeMail, { isLoading: subscribing }] = useContactMutation();
    ...
    const handleNewsletterSubmit = async () => {
        ...
        try {
            await subscribeMail({ email: newsletterEmail }).unwrap();
            setNewsletterEmail('');
        } catch {
    ...
    /* ── Derived data ── */
    const displayPosts = useMemo(() => [...posts].reverse().slice(0, visibleCount), [posts, visibleCount]);
    ...
    return (
        <>
            ...
            <link
                href="https://fonts.googleapis.com/css2?family=Anton&family=Space+Mono:ital@0;1&display=swap"
                rel="stylesheet"
            />
            ...
            <div className="blog-page">
                ...
                <section className="blog-hero">
                    ...
                    <div className="blog-hero-inner">
                        <div className="blog-breadcrumb">
                            <Link href="/" className="blog-bc-link">Home</Link>
                ...
                <div className="blog-layout">
                        ...
                        {error && (
                            <div className="blog-error">
                                ...
                                <div>
                                    ...
                                    <p className="blog-error-msg">Something went wrong. Try refreshing.</p>
                                </div>
                            </div>
                        )}
                            ...
                            <article className="blog-card-featured">
                                ...
                                <div className="blog-card-featured-body">
                                    <div className="blog-card-featured-meta">
                                        <span className="blog-card-meta-item">👤 &nbsp; Admin</span>
                                    ...
                                    <Link href={`/blog/${featuredPost._id}`} className="blog-card-featured-title">
                                    ...
                                    <div style={{ display: 'flex', gap: '.75rem', alignItems: 'center' }}>
                                        <Link href={`/blog/${featuredPost._id}`} className="blog-card-cta">
                                            Read Article →
                                        </Link>
                                        <Link href="/blog" className="blog-card-cta-ghost">
                                            All Posts
                                        </Link>
                                    </div>
                                </div>
                        ...
                        {!loading && regularPosts.map((post, index) => (
                            <article className="blog-card" key={post._id || index}>
                                <div className="blog-card-img-wrap">
                                    <img src={getPostImage(post, index + 1)} alt={post.title || 'Post image'} />
                                    <div className="blog-card-img-overlay" />
                                </div>
                                <div className="blog-card-body">
                                    <span
                                        className="blog-card-cat-tag"
                                        style={{ ['--tag-color' as string]: TAG_COLORS[index % TAG_COLORS.length] }}
                                    >
                                        {post.category || 'Articles'}
                                    </span>
                                    <Link href={`/blog/${post._id}`} className="blog-card-title">
                                    ...
                                    <div className="blog-card-footer">
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                            ...
                                            <Link href={`/blog/${post._id}`} className="blog-card-read-link">
                                                Read More →
                                            </Link>
                                        </div>
                                    </div>
                    ...
                    <aside className="blog-sidebar">
                        {/* Search */}
                        <div className="blog-sb-card" style={{ ['--sb-accent' as string]: T.red }}>
                        ...
                        {/* Categories */}
                        <div className="blog-sb-card" style={{ ['--sb-accent' as string]: T.orange }}>
                            <div className="blog-sb-head">
                                    ...
                                    categories.map((cat) => {
                                        ...
                                        return (
                                            <Link
                                                key={id}
                                                href="/blog"
                                                className={`blog-sb-cat${categoryId === id ? ' active' : ''}`}
                                                onClick={(e) => { e.preventDefault(); handleCategoryFilter(id!); }}
                                                style={{ ['--sb-accent' as string]: T.orange }}
                                            >
                        ...
                        {/* Recent Posts */}
                        <div className="blog-sb-card" style={{ ['--sb-accent' as string]: T.teal }}>
                            <div className="blog-sb-head">
                                <h3 className="blog-sb-title">Recent Posts</h3>
                            </div>
                            <div className="blog-sb-body">
                                {posts.slice(0, 4).map((post, i) => (
                                    <Link key={post._id || post.id || i} href={`/blog/${post._id || post.id}`} className="blog-recent-item">
                                        <img
                        ...
                        <div className="blog-sb-card" style={{ ['--sb-accent' as string]: T.orange }}>
                            ...
                            <div className="blog-sb-body">
                                ...
                                />
                                <button className="blog-sb-newsletter-btn" onClick={handleNewsletterSubmit} disabled={subscribing}>
