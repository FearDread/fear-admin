...
__export(index_exports, {
  ApiFactory: () => service_default,
  FeatureFactory: () => factories_default,
  StateFactory: () => state_default,
  ThunkFactory: () => thunk_default
});
...
var ThunkFactory = {
  create: (entity, prefix) => {
    return (0, import_toolkit.createAsyncThunk)(
      `${entity}/${prefix}`,
...
function FeatureFactory(entity, reducers = {}, endpoints = null) {
  const factory = {
    ...
    thunk: thunk_default,
    state: StateFactory,
    adapter: (0, import_toolkit2.createEntityAdapter)()
  };
  factory.manager = (initialReducers = {}) => {
    const reducersMap = { ...initialReducers };
    let combined = (0, import_toolkit2.combineReducers)(reducersMap);
    return {
      reduce: (state, action) => combined(state, action),
      add: (key, reducer) => {
        if (!key || reducersMap[key]) return;
        reducersMap[key] = reducer;
        combined = (0, import_toolkit2.combineReducers)(reducersMap);
      },
      remove: (key) => {
        if (!key || !reducersMap[key]) return;
...
var ApiFactory = (sliceName) => {
  ...
  _this.baseApi = (0, import_react.createApi)({
    reducerPath: sliceName,
    tagTypes: ["Product", "Order", "User", "Category"],
    baseQuery: (0, import_react.fetchBaseQuery)(_this.baseQuery),
    endpoints: () => ({})
  });
  _this.create = (routes) => {
