...
import { selectCurrentOrder, createOrder } from '../../features/orders/slice';
...
function CheckoutReview() {
  ...
  useEffect(() => {
    if (!currentOrder?.shippingAddress?.line1) { navigate('/checkout/details'); return; }
    if (!currentOrder?.paymentIntentId)         navigate('/checkout/payment');
  }, [currentOrder, navigate]);

  const handleCompleteOrder = async () => {
    if (!currentOrder?.paymentIntentId) { navigate('/checkout/payment'); return; }
    ...
    try {
      const finalOrder = {
        ...
        items: cartItems.map(item => ({
          ...
          subtotal: item.price, quantity: item.quantity, image: item.image,
        })),
      ...
      navigate('/checkout/complete', { state: { order: result || finalOrder } });
    } catch (err) {
      console.error(err);
    } finally {
      ...
      <div className="co-crumb-bar">
        <div className="efear-container co-crumb-nav">
          <nav className="co-crumb-trail">
            ...
            <span className="co-crumb-sep">›</span>
            <Link to="/checkout/payment" className="co-crumb-link">Payment</Link>
            <span className="co-crumb-sep">›</span>
          ...
          <div className="co-layout">
                ...
                <div className="co-review-items">
                  {cartItems.map(item => (
                    <div key={item.productId} className="co-review-item">
                      <img src={item.image || 'assets/images/products/placeholder.png'} alt={item.name || item.title} className="co-review-img" />
                      <div>
                        <p className="co-review-name">{item.name || item.title}</p>
              ...
              <div className="co-panel" style={{ marginTop: '1rem' }}>
                ...
                <div className="co-info-grid">
                  ...
                  <div className="co-info-block">
                    ...
                    <p className="co-info-line"><strong>Credit Card</strong>Payment processed securely via Stripe</p>
                    {currentOrder?.paymentIntentId && (
                      <div className="co-payment-badge">✓ Payment Authorised</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="co-panel-body" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontFamily: "'Space Mono',monospace", fontSize: '.65rem', color: 'rgba(255,255,255,0.32)', margin: 0, letterSpacing: '.08em' }}>
                      Est. delivery: {currentOrder?.estimatedDelivery || '2-5 business days'}
                    </p>
                  </div>
                </div>
                <div className="co-nav-btns">
                  <button onClick={() => navigate('/checkout/payment')} disabled={isProcessingOrder} className="cart-dd-btn-ghost" style={{ padding: '.65rem' }}>← Back to Payment</button>
                  <button onClick={handleCompleteOrder} disabled={isProcessingOrder || cartItems.length === 0} className="cart-dd-btn-primary" style={{ padding: '.65rem' }}>
                    {isProcessingOrder ? (
                      <><span className="co-pay-spinner" style={{ width: '14px', height: '14px', marginRight: '.5rem', display: 'inline-block' }} />Processing...</>
                    ) : 'Complete Order →'}
                  </button>
                </div>
              </div>
            ...
            <div className="co-sidebar">
              <div className="co-sidebar-panel accent-top">
                ...
                <button onClick={handleCompleteOrder} disabled={isProcessingOrder || cartItems.length === 0} className="auth-submit" style={{ marginTop: 0 }}>
